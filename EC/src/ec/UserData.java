...
public class UserData extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			List<BuyDataBeans> bdblist = BuyDAO.getBuyDataBeansByBuyUserId(userId);
			...
			ArrayList<ItemDataBeans> udblist = BuyDetailDAO.getItemDataBeansListByBuyId(userId);
			// 入力された内容に誤りがあったとき等に表示するエラーメッセージを格納する.	↓の
			String validationMessage = (String) EcHelper.cutSessionAttribute(session, "validationMessage");

			request.setAttribute("validationMessage", validationMessage);
			request.setAttribute("udb", udb);
			request.setAttribute("ublist", udblist);
			request.setAttribute("ubdlist", udblist);
